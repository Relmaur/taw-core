...
- **Content interchange (`TAW\Core\Content\*`) — the portable snapshot / change-set subsystem.** `Exporter::snapshot()` builds a `json_encode`-ready array (posts + `_taw_*` fields, `_taw_*` options, terms, referenced media); `Importer::plan()` is a mandatory zero-write dry run, `Importer::apply()` writes a rollback snapshot to `uploads/taw-private/` first. Records matched by natural key (post `type`+`slug`, option key, term `taxonomy`+`slug`) — never numeric ID. `FieldCodec` is the pure decode/attachment-ref layer (unit-tested without WP). CLI: `content:export` / `content:import` / `content:diff`. Schema: `resources/schema/content-interchange-1.0.json`. Wired default-on in `Theme::boot()` step 13 (Tools → TAW Data screen, `GET taw/v1/content/export`, and `FieldMetaRegistrar` — REST-registered field meta; `add_filter('taw_register_meta_in_rest', '__return_false')` opts out the last).
